<template>
  <Layout>
    <div class="content-wrapper">
      <div class="title">
        <router-link to="/tags"><Icon name="arrow-left" /></router-link>
        <div>{{ original }}</div>
        <div><Icon name="right" /></div>
      </div>
      <FormInput
        title="新的标签名"
        :value="tagname"
...
export default class Edit extends Vue {
  ...
  modifyTag() {
    ...
    if (flag === true) {
      if ("" === this.tagname) {
        ...
        if (this.original === this.tagname) {
          ...
          if (this.tagname.length > 8) {
            ...
            if (tagListModel.fetch().indexOf(this.tagname) >= 0) {
              window.alert("你已经保存过这个标签，重复了！");
            } else {
              let d = this.deleteProcess();
              d.then((res) => {
                if (res.code === 0) {
                  let addResult = tagListModel.create(this.tagname);
                  if (addResult.code === 0) {
                    window.alert("修改成功！");
                    this.$router.replace("/tags");
                  } else {
      ...
      //   window.alert("标签名不能为空，再想想？");
      // } else {
      //   let deleteResult = tagListModel.delete(this.original);
      ...
      //       } else {
      //         window.alert("修改失败" + addResult.code + addResult.message);
      //       }
      //     }
      //   } else {
      //     window.alert(
      //       "无法删除标签：" + deleteResult.code + deleteResult.message
      //     );
      //   }
      // }
    } else {
  ...
  created() {
    if (tagListModel.fetch().indexOf(this.$route.params.tagname) >= 0) {
